...
1. Keep GOAL hunters as **siblings of `surface-split`** (`split >> hunters`), not a `hunter-k` → `hunter-(k+1)` `depends_on` chain. Agentflow skips dependents of a FAILED node (`upstream_failure`) even when `fail_fast` is False, so a hunter-1 timeout must not be on hunter-2's `depends_on` (QA `cmtuvv61w0003gm0az74grqv2`). Run them in parallel with `concurrency=N` (`GOAL_COUNT`). `#12` serialized with `concurrency=1` to dodge new-account 429; that is what made ReactFlow look serial (run `cmtvcakz70003ku0407pg6bfz`). The gate is **exit-0 hunter wraps** plus **`judge-a depends_on` every hunter**. A hunter timeout must not mark the graph node FAILED — that would skip the judge and fail the whole GOAL run (`ecs_task_failed` + refund) even if `report.md` published.
2. Do **not** lower `max_tokens` further (default remains **16384**).
3. Defense in depth: `wrap_kimi` OpenRouter proxy retries **429** up to 8 times, honoring `Retry-After` through **90s** (a 20 RPM window is 60s) or exponential backoff (1s … 90s; cap 90s). 402 is not retried. Parallel hunters are the intended schedule; retries wait out an in-node / fan-out kimi burst.
...
5. **hunter-1** … **hunter-N** — first-class dynamic nodes from `GOAL_COUNT` (default 6). **Siblings of `surface-split`** (agentflow `depends_on`); **parallel via `concurrency=N`**. Graph `fail_fast=False` so one unexpected hunter FAIL does not skip hunters N+1… (QA `cmtuvv61w0003gm0az74grqv2`: hunter-1 `Timed out after 1800s`, empty RESULT.md, fail_fast killed the GOAL run). Wrap (`MIDKERNEL_HUNTER_CONTINUE=1`) kills kimi at the soft deadline (90% of hard), writes incomplete `RESULT.md` if needed, and **exits 0** so the graph node is COMPLETED. OpenRouter 429 new-account RPM (`cmtun51000003l704q7lyyjrf`) is mitigated by `wrap_kimi` Retry-After, not by serializing hunters. Each picks `goals/0N-*.md` if present and no-ops cleanly if missing. Soft deadline is 90% of the hard node timeout (low: **27 minutes** / 1620s). Candidates go under `findings/hunter-N/`. **No** known-issues / GitHub issue-or-PR duplicate search. Live `graph.json` draws the same fan-out `surface-split → hunter-1…N` plus each hunter → `judge-a`.
