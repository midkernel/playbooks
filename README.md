...
4. The graph then runs inside that task. Default playbooks (`security-review`, Solana, Firedancer) are **prepare** → **review** → **publish**. `goal-security-review` is **prepare** → **threat-model** → **goal-author** → **surface-split** → **hunter-1 … hunter-N** (`GOAL_COUNT`, default 6; **siblings of `surface-split`**, `concurrency=1`, `fail_fast=False`) → **`hunter-join`** (cycle barrier) → **judge-a** → **judge-b** → **assemble** → **publish**. A hunter hard-fail (timeout / non-zero) must not skip later hunters or judges. `judge-a` must not become ready until every hunter is COMPLETED or FAILED — declaration order / `concurrency=1` FIFO is **not** that guarantee (pinned agentflow `09df0175` ready set is a hash-ordered `set`). Missing, empty, or stub `report.md` fails publish; the final report key is not written.
...
5. Hunter success criteria still require `findings/hunter-N/RESULT.md`. Wrap uploads that file when the model actually writes it. Wrap does **not** invent findings on 402 / empty return unless `MIDKERNEL_HUNTER_CONTINUE=1` (GOAL hunters): then it writes a clearly marked **incomplete** `RESULT.md` (not a finding), keeps UI meta `failed`, and **exits 0** so the graph node is COMPLETED when kimi is not SIGKILL'd. Agentflow hard timeout still FAILS the node; `hunter-join` covers that.
...
1. Keep GOAL hunters serialized with graph `concurrency=1`. They are **siblings of `surface-split`** (`split >> hunters`), not a `hunter-k` → `hunter-(k+1)` `depends_on` chain. Agentflow skips dependents of a FAILED node (`upstream_failure`) even when `fail_fast` is False, so a hunter-1 timeout must not be on hunter-2's `depends_on` (QA `cmtuvv61w0003gm0az74grqv2`). Do **not** raise `concurrency` — parallel hunters are the OpenRouter 429. `concurrency=1` does **not** order the ready set (Eng review FAIL on playbooks #16): after `surface-split`, hunters **and** a `surface-split`-parented `judge-a` are all ready; FIFO is arbitrary set order, so judges can score empty `findings/hunter-*/`. The gate is **`hunter-join`**: `on_failure >> hunter-join` (cycle) waits for every hunter COMPLETED|FAILED, always exits 0, and `judge-a` `depends_on` only that barrier.
2. Do **not** lower `max_tokens` further (default remains **16384**).
3. Defense in depth: `wrap_kimi` OpenRouter proxy retries **429** up to 8 times, honoring `Retry-After` through **90s** (a 20 RPM window is 60s) or exponential backoff (1s … 90s; cap 90s). 402 is not retried. Serialization remains the primary fix; retries wait out an in-node kimi burst.
...
5. **hunter-1** … **hunter-N** — first-class dynamic nodes from `GOAL_COUNT` (default 6). **Siblings of `surface-split`** (agentflow `depends_on`); **serialized by `concurrency=1`**. Graph `fail_fast=False` so one hunter timeout / non-zero exit does not skip hunters N+1… (QA `cmtuvv61w0003gm0az74grqv2`: hunter-1 `Timed out after 1800s`, empty RESULT.md, fail_fast killed the GOAL run). Do not run them in parallel — that is the OpenRouter 429 new-account RPM failure (`cmtun51000003l704q7lyyjrf`). Each picks `goals/0N-*.md` if present and no-ops cleanly if missing. Soft deadline is 90% of the hard node timeout (low: **27 minutes** / 1620s). Candidates go under `findings/hunter-N/`. **No** known-issues / GitHub issue-or-PR duplicate search. Live `graph.json` still draws a serialized UI chain `surface-split → hunter-1 → … → hunter-N` plus each hunter → `judge-a` (`hunter-2..N never regain a surface-split edge` in the UI).
6. **hunter-join** — always-success **cycle barrier** (`on_failure >> hunter-join`). Pinned agentflow 09df0175 skips ordinary nodes when any `depends_on` parent is FAILED (`upstream_failure`), so judges cannot hard-`depends_on` hunters. The join becomes ready only when every hunter is COMPLETED or FAILED, then exits 0. This is the mechanical gate: `judge-a` is not in the ready set until `hunter-join` is COMPLETED. Declaration order is not a scheduler.
7. **judge-a** — security-relevance vs `THREAT_MODEL.md` (default `OPENROUTER_MODEL` / balanced Pareto Scan preset `google/gemini-3.8-flash`). Survivors → `findings/validated-a/`. `depends_on` is **`hunter-join` only**.
8. **judge-b** — PoC / exploitability on a **different** OpenRouter model (default `anthropic/claude-sonnet-4.5`). Survivors → `findings/validated-b/`.
9. **assemble** — only dual-pass survivors → `report.md`. Empty findings with evidence of what was tried is allowed. Never invent. No stub language.
10. **publish** — same `PUBLISH_SCRIPT` (nonempty, non-stub `report.md` or fail closed).
