...
4. The graph then runs inside that task. Default playbooks (`security-review`, Solana, Firedancer) are **prepare** → **review** → **publish**. `goal-security-review` is **prepare** → **threat-model** → **goal-author** → **surface-split** → **hunter-1 … hunter-N** (`GOAL_COUNT`, default 6; **siblings of `surface-split`**, `concurrency=1`, `fail_fast=False`) → **judge-a** (`depends_on` every hunter) → **judge-b** → **assemble** → **publish**. GOAL hunters wrap to graph **COMPLETED** (exit 0 + nonempty `RESULT.md`) even on kimi timeout / non-zero; UI meta still records incomplete/failed. That is the hunter→judge barrier **and** the product settle: a hunter timeout must not mark the graph node FAILED (any FAILED node → agentflow `RunStatus.FAILED` → CLI exit 1 → runner `GraphError` → app `ecs_task_failed` + refund even if publish wrote `report.md`). Declaration order / `concurrency=1` FIFO is **not** a scheduler (pinned agentflow `09df0175` ready set is a hash-ordered `set`). Missing, empty, or stub `report.md` fails publish; the final report key is not written.
...
5. **hunter-1** … **hunter-N** — first-class dynamic nodes from `GOAL_COUNT` (default 6). **Siblings of `surface-split`** (agentflow `depends_on`); **serialized by `concurrency=1`**. Graph `fail_fast=False` so one unexpected hunter FAIL does not skip hunters N+1… (QA `cmtuvv61w0003gm0az74grqv2`: hunter-1 `Timed out after 1800s`, empty RESULT.md, fail_fast killed the GOAL run). Wrap (`MIDKERNEL_HUNTER_CONTINUE=1`) kills kimi at the soft deadline (90% of hard), writes incomplete `RESULT.md` if needed, and **exits 0** so the graph node is COMPLETED. Do not run hunters in parallel — that is the OpenRouter 429 new-account RPM failure (`cmtun51000003l704q7lyyjrf`). Each picks `goals/0N-*.md` if present and no-ops cleanly if missing. Soft deadline is 90% of the hard node timeout (low: **27 minutes** / 1620s). Candidates go under `findings/hunter-N/`. **No** known-issues / GitHub issue-or-PR duplicate search. Live `graph.json` still draws a serialized UI chain `surface-split → hunter-1 → … → hunter-N` plus each hunter → `judge-a` (`hunter-2..N never regain a surface-split edge` in the UI).
