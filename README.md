...
3. `wrap_kimi` rewrites a non-file `--config`, writes that TOML (including `max_tokens = 16384` next to `max_output_size`, lockstep with runner), starts a localhost proxy that **never forwards** `chat/completions` without a capped `max_tokens` on the JSON body (empty / missing Content-Length / missing field → inject; non-JSON → 400), retries OpenRouter **429** (Retry-After / exponential backoff, cap 4), and forwards the OpenRouter env to `MIDKERNEL_KIMI_BIN`.
...
3. Defense in depth: `wrap_kimi` OpenRouter proxy retries **429** up to 4 times, honoring `Retry-After` or exponential backoff (1s, 2s, 4s, 8s; cap 32s). 402 is not retried.
