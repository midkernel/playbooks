...
4. The graph then runs inside that task. Default playbooks (`security-review`, Solana, Firedancer) are **prepare** → **review** → **publish**. `goal-security-review` is **prepare** → **threat-model** → **goal-author** → **surface-split** → **hunter-1 → hunter-2 → … → hunter-N** (`GOAL_COUNT`, default 6; **serialized** `depends_on` chain, `concurrency=1`) → **judge-a** → **judge-b** → **assemble** → **publish**. Missing, empty, or stub `report.md` fails publish; the final report key is not written.
...
1. Serialize GOAL hunters with a `depends_on` chain: `hunter-1` depends on `surface-split`, `hunter-k` depends on `hunter-(k-1)`. Graph `concurrency=1`. Agentflow has no hunter-group concurrency knob on the pinned ref; the chain is the supported mechanism. Do **not** restore `split >> hunters`.
